...
var destination string = "/bigdata/students/gakulkarni/"
...
func listenForControllerMsg() {
	listener, err := net.Listen("tcp", ":"+"9999")
	helper.CheckErr(err)
	fmt.Println("Storage node Component listening on port for Controller:: ", 9999)
	helper.CheckErr(err)
	for {
		if conn, err := listener.Accept(); err == nil {
			clientHandler := client_handler.NewFileHandler(conn)
			handleControllerMsg(clientHandler)
		}
	}
}

func handleControllerMsg(clientHandler *client_handler.FileHandler) {
	...
	fmt.Println("Handling controller request")
	if statusArr[0] == "Replicate" {
		fmt.Println("Controller asked to maintain replication level")
		_, err := os.Stat(destination + statusArr[3])

		if err == nil {
			localchunkname := statusArr[3]
			localchunkchecksumname := ""
			// Read the file as bytes
			fmt.Println("**** File found *****\n sending file to ", statusArr[1])
			fileBytes, err := ioutil.ReadFile(destination + localchunkname)
			helper.CheckErr(err)
			if strings.HasSuffix(localchunkname, "_r") {
				localchunkchecksumname = strings.TrimRight(localchunkname, "_r") + "_checksum_r"
			} else {
				localchunkchecksumname = localchunkname + "_checksum"
			}
			checksumBytes, err := ioutil.ReadFile(destination + localchunkchecksumname)
			helper.CheckErr(err)
			checksum := string(checksumBytes)
			fmt.Println("Sending file ", filename, " to ", statusArr[1])
			sendReplicaToStorageNode(statusArr[1]+":"+statusArr[2], &client_handler.ClientMsg{Data: fileBytes, Action: "replicate", Filename: filename, Checksum: checksum})
		} else {
...
func handleStorageNode(storageHandler *client_handler.FileHandler, waitg *sync.WaitGroup) {
	...
	case action == "replicate":
		replicateFile(response)
	default:
		fmt.Println("Unexpected action. Please choose between get/put")
	}
}
func replicateFile(response *client_handler.ClientMsg) {
	fmt.Println("Handling replication level")
	checksumfilename := ""
	if strings.HasSuffix(response.GetFilename(), "_r") {
		checksumfilename = strings.TrimRight(response.GetFilename(), "_r")
		checksumfilename += "_checksum_r"
	} else {
		checksumfilename = response.GetFilename() + "_checksum"
	}
	fmt.Println("Storing checksum with filename ", checksumfilename)
	//Write checksum to a file
	err := os.WriteFile(destination+checksumfilename, []byte(response.GetChecksum()), 0644)
	helper.CheckErr(err)
	fmt.Println("Replica Checksum file saved on server successfully.")
	fmt.Println("Replicated Checksum::", response.GetFilename())

	fmt.Println("Storing file with filename ", response.GetFilename())
